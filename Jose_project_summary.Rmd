...
After generating the transgenic line, we first analysed the oil content in seeds from wildtype (**IT86**) and transgenic (**15B**) seeds during seed developmet (Figure 1). The transgenic line showed a significant increased in oil content from stage 3, remaining high until stage 5 (matured dry seed). 
...
heatmap_plot <- heatmap_plot +  
  labs(x = "Sample developmental stage",
       y = "Genes",
...
# My Digital Toolbox
For this project I have used mainly the `Tydyverse` package, but I had to add a few others to add images to graphs (`library(png)` and `library(grid)`) I also explored the `heatmap()` function that is built in R for using a matrix of data, although I decied to use `ggplot` at the end for making the heatmap.
...
# My time went ...
I spend about 10% of the time tidying the data, about 80% trying to do the cluster analysis and heatmap and about 10% preparing the Rmarkdown file. I found it a bit overwhelming because there where so many options when I seached for help. It was difficult to change from dataframes to matrix formats. Also because I tried to analyses the whole cowpea genome I was running out of computing power. Likely, the instructors put me into the right track and I could find the right approach.

# Next steps
After this exploratory work, I need now to keep playing with the pre-processing filtering steps and with the clustering parameters to find the genes more likely to be biologically relevant for oil accumulation.
