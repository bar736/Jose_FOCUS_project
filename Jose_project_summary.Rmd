...
# My Project
My project is about exploring gene changes in a **transgenic "high-oil" cowpea** line that we have made. Cowpea is a key staple crop in in West Africa being a main  protein source for over 200 million people. By introducing the gene cassette that appears below with three oil-related genes we tried to increased the oil-content in the seeds making them more nutritious.

Our transgenic line (**named 15B**) has more oil in the seeds that the wildtype line (**named IT86**). We collected RNA samples from developing seeds from 15B and IT86 at **4 different stages** and performed a trancriptomic RNAseq analysis. 
...
![](resources/img/cowpea.jpg){width=200px} ![](resources/img/oilconstruct.jpg){width=700px}



## Description of the samples
We first analysed the oil content in seeds from wildtype and transgenic seeds during seed developmet (Figure 1). The transgenic line showed a significant increased in oil content from stage 3, remaining high until stage 5 (matured dry seed). 
We performed RNAseq in stages 1 to 4 in both genotypes. 
...
## Workflow
I started with a matrix of expression values and first I have to transform the data into a tidy dataframe and add the gene descriptions into it. After that I did some basic data processing to remove genes with low expression, transform the expression and normlise it.

After that I performed a cluster analysis using k-means clustering (`cluster::clara()`) with an arbitrary number of clusters (k=10) to get a quick look at expression patterns. It was a bit tricky because for this kind of analysis I have to change between dataframes and matrices, but I got there at the end (Table 1).
...
knitr::kable(head(expn_scaled, n = 5), format = "html", caption = "First rows of the final table containing the clustered relative expression values for each gene and the annotations") %>% 
  kable_styling("striped")
```

## Results
Once I obtained the data frame containing the clustered genes with relative expression values I used `ggplot()` to built the heatmap.

```{r standard-plot2, echo = FALSE, out.width='60%', fig.align='center', fig.height= 8, fig.width=6, fig.cap="Gene expression heatmap during seed developmet"}
...
# My Digital Toolbox
For this project I have used mainly the `Tydyverse` package, but I had to add a few others to add images to graphs and also for trying the `heatmap()` function using a matrix.

## Favourite tool

![](resources/img/tidyverse.png){width=100px}![](resources/img/ggplot.png){width=90px}


# My time went ...

I spend about 10% of the time tidying the data, and about 90% trying to do the cluster analysis and heatmap. I found it a bit overwhelming because there where so many options when I seached for help. It was difficult to change from dataframes to matrix formats. Also because I tried to analyses the whole cowpea genome I was running out of computing power. Likely, with a bit of help from the instructors I could find the right approach.

# Next steps

After this exploratory work, I need now to keep playing with the pre-processing steps and with the clustering parameters to find the genes more likely to be biologically relevant for oil accumulation.
...
I am very happy to have attended **Data School**. The experiencxe has been much more positive than I though and, starting from 0, I have learnt heaps. The course was very well structured and the teachers were fantastic. I will definetly recommend it.
